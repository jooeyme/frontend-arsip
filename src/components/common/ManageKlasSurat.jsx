...
import { getAllKlasifikasi, createKlasifikasi, deleteKlasifikasi } from "../../modules/fetch/klasifikasi";
...
export default function ManageKlasSurat() {
  const navigate = useNavigate();
  const [klasifikasi, setKlasSurat] = useState([]);
  ...
  const fetchData = async () => {
    try {
      const response = await getAllKlasifikasi();
      setKlasSurat(response);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
        const res = await createKlasifikasi(formData);
        setKlasSurat((prev) => [...prev, res.data]);
        setOpen(false);
        fetchData();
        setFormData({
            kode: "",
  ...
  const handleDeleteKlasSurat = (id) => {
    Swal.fire({
                  ...
                  klasifikasi.map((klas, index) => (
                    <TableRow key={klas.id}>
                      ...
                      <TableCell className="px-5 py-4 sm:px-6">
                        <Button size="sm" variant="delete" onClick={() => handleDeleteKlasSurat(klas.id)}>
                          Delete
                        </Button>
                      </TableCell>
                    </TableRow>
            ...
            <div className="custom-scrollbar h-[450px] overflow-y-auto px-2 pb-3">
              <div className="mt-7">
                ...
                <div className="grid grid-cols-1 gap-x-6 gap-y-5">
                  ...
                  <div className="">
                    <Label htmlFor="deskripsi">Deskripsi Klasifikasi</Label>
                    <Input
                      type="text"
                      id="deskripsi"
                      name="deskripsi"
