...
import { getAllTujuan, createTujuan, deleteTujuan } from "../../modules/fetch/tujuan";
...
export default function ManageTujuan() {
  ...
  const fetchData = async () => {
    try {
      const response = await getAllTujuan();
      setTujuan(response);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
        const res = await createTujuan(formData);
        setTujuan((prev) => [...prev, res.data]);
        setOpen(false);
        fetchData();
        setFormData({
            nama: "",
                  ...
                  tujuan.map((klas, index) => (
                    <TableRow key={klas.id}>
                      ...
                      <TableCell className="px-5 py-4 sm:px-6">
                        <Button size="sm" variant="delete" onClick={() => handleDeleteTujuan(klas.id)}>
                          Delete
                        </Button>
                      </TableCell>
                    </TableRow>
