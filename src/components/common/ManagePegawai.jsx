...
import { getAllPegawai, createPegawai, deletePegawai } from "../../modules/fetch/pegawai";
...
export default function ManagePegawai() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
        const res = await createPegawai(formData);
        setPegawais((prev) => [...prev, res.data]);
        setOpen(false);
        fetchData();
        setFormData({
            nama_lengkap: "",
                  ...
                  pegawais.map((user, index) => (
                    <TableRow key={user.id}>
                      ...
                      <TableCell className="px-5 py-4 sm:px-6">
                        <Button size="sm" variant="delete" onClick={() => handleDeletePegawai(user.id)}>
                          Delete
                        </Button>
                      </TableCell>
                    </TableRow>
